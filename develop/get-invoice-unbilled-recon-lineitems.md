...
---
# <a name="get-invoices-unbilled-reconciliation-line-items"></a>Pobieranie nierozliczanych elementów wierszy uzgadniania faktur
...
Korzystając z poniższych metod, można uzyskać zbiór szczegółów dotyczących nienaliczanych elementów wierszy faktury (nazywanych również otwartymi wierszami rozliczeń).
...
Aby uzyskać elementy wiersza dla określonej faktury, Pobierz obiekt Invoice:
...
Obiekt Invoice zawiera wszystkie informacje dotyczące określonej faktury:

- **Dostawca** identyfikuje źródło informacji o nieobciążanych szczegółach (na przykład **jednorazowej**).
...
3. Utwórz moduł wyliczający, który przejdzie do kolekcji. Aby zapoznać się z przykładem, zobacz następujący przykładowy kod.

Następujący przykładowy kod używa pętli **foreach** do przetwarzania kolekcji **InvoiceLineItems** . Dla każdego **InvoiceLineItemType** pobierana jest oddzielna Kolekcja elementów wiersza.
...
|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Pobierz** | [*{baseURL}*](partner-center-rest-urls.md)/V1/Invoices/{Invoice-ID}/LineItems? Provider = jednorazowej&invoicelineitemtype = billinglineitems&CurrencyCode = {currencycode} &okres = {period} http/1.1                              | Użyj tej składni, aby zwrócić pełną listę każdego elementu wiersza dla danej faktury. |
| **Pobierz** | [*{baseURL}*](partner-center-rest-urls.md)/V1/Invoices/{Invoice-ID}/LineItems? Provider = jednorazowej&invoicelineitemtype = billinglineitems&CurrencyCode = {currencycode} &period = {period} &size = {size} http/1.1  | W przypadku dużych faktur należy użyć tej składni z określonym rozmiarem i przesunięciu na 0, aby zwrócić stronicowaną listę elementów wiersza. |
| **Pobierz** | [*{baseURL}*](partner-center-rest-urls.md)/V1/Invoices/{Invoice-ID}/LineItems? Provider = jednorazowej&invoicelineitemtype = billinglineitems&CurrencyCode = {currencycode} &period = {period} &size = {size} &SeekOperation = Next                               | Użyj tej składni, aby uzyskać następną stronę elementów wiersza uzgodnienia przy użyciu `seekOperation = "Next"` . |
...
|------------------------|--------|----------|---------------------------------------------------------------------------------|
| Identyfikator faktury             | ciąg | Tak      | Ciąg, który identyfikuje fakturę. Użyj "unbilld", aby uzyskać rozliczane oszacowania. |
| dostawcy               | ciąg | Tak      | Dostawca: "jednorazowej".                                                |
| Typ faktury-wiersz-element | ciąg | Tak      | Typ szczegółów faktury: "BillingLineItems".               |
| hasPartnerEarnedCredit | bool   | Nie       | Wartość wskazująca, czy zwrócić elementy wiersza z zastosowaniem środków uzyskanych przez partnera. Uwaga: ten parametr zostanie zastosowany tylko wtedy, gdy typ dostawcy to jednorazowej, a InvoiceLineItemType to UsageLineItems.
| currencyCode           | ciąg | Tak      | Kod waluty dla nieobciążanych elementów wiersza.                                  |
| period                 | ciąg | Tak      | Okres nienaliczanych rekonesans. przykład: Current, Previous.                      |
