...
| NotImplemented      | 501       | 0          | Nie zdefiniowano wyjątku       |
...
| BadRequest          | 400       | 2012       | Nie można przekonwertować między identyfikatorami subskrypcji Azure Active Directory handlowej i subskrypcji.          |
...
| BadRequest          | 400       | 27007      | Nieprawidłowy kod podsyłania         |
...
| NotFound      | 404       | 2072       | Identyfikator MPN dostawcy pośredniego jest nieprawidłowy jako zarejestrowanego partnera          |
...
| Forbidden     | 403       | 13605      | Zanim będzie można ukończyć ten zakup, Umowa z Klientem Microsoft musi zostać dostarczone potwierdzenie akceptacji klienta przez partnera lub klient musi zaakceptować Umowa z Klientem Microsoft w Centrum administracyjnym firmy Microsoft           |
...
| BadRequest          | 400       | 600002     | Wartość identyfikatora rejestracji organizacji nie jest obsługiwana  <br/><br/>Ten błąd występuje, gdy firma/organizacja  klienta nie znajduje się w jednym z następujących krajów, ale próbowali określić organizationRegistrationNumber. Kraje, których to dotyczy:<br/><br/>— Zamów (AM) <br/>— Zakłotek (AZ)<br/>- Wniesienie (BY)<br/>— Dobowe (HU)<br/>- Wz (KZ)<br/>- Kyrgyzstan (KG)<br/>— Dobowe (MD)<br/>— Rosyjski (RU)<br/>- Tadżykistan (TJ)<br/>- Dobrzy (UZ)<br/>- W rzece (UA) |
| BadRequest          | 400       | 600049     | Brak informacji o identyfikatorze rejestracji organizacji <br/><br/>Ten błąd występuje, jeśli firma/organizacja klienta znajduje się w jednym z następujących krajów i nie podano organizacjiRegistrationNumber. Kraje, których to dotyczy:<br/><br/>— W Szmionie (AM) <br/>— Wzręski (AZ)<br/>- Wniesienie (BY)<br/>— Dobowe (HU)<br/>- Wz (KZ)<br/>- Kyrgyzstan (KG)<br/>— Dobowe (MD)<br/>— Rosyjski (RU)<br/>- Tadżykistan (TJ)<br/>- Dobrzy (UZ)<br/>- W rzece (UA)       |
...
| BadRequest          | 400       | 600051     | Brak numeru telefonu klienta <br/><br/>Ten błąd występuje, jeśli firma/organizacja klienta znajduje się w jednym z następujących krajów, ale nie podano wartości billingProfile.defaultAddress.phoneNumber. Kraje, których to dotyczy:<br/><br/>— W Szmionie (AM) <br/>— Wzręski (AZ)<br/>- Wniesienie (BY)<br/>— Dobowe (HU)<br/>- Wz (KZ)<br/>- Kyrgyzstan (KG)<br/>— Dobowe (MD)<br/>— Rosyjski (RU)<br/>- Tadżykistan (TJ)<br/>- Dobrzy (UZ)<br/>- W rzece (UA)       |
...
| Forbidden     | 403       | 800073     | Twoje konto klienta jest obecnie w trakcie przeglądu. Zaczekaj kilka dni. W tym czasie dziękujemy za cierpliwość. Dowiedz się więcej na stronie {0}              |
| Forbidden     | 403       | 800074     | Twoje konto klienta jest obecnie w trakcie przeglądu. Zaczekaj kilka dni. W tym czasie dziękujemy za cierpliwość. Dowiedz się więcej na stronie {0}              |
...
| BadRequest          | 400       | 800082     | do zaktualizowania. Spróbuj ponownie po pewnym czasieSubskrypcja nie jest gotowa      |
| InternalServerError | 500       | 800083     | Dostępność ma więcej niż jedną uwzględnioną opcje ilości         |
...
| Forbidden     | 403       | 800088     | Żądana liczba stanowisk przekroczyła pozostały limit dozwolonych stanowisk {0} {1} na subskrypcję dla wartości CatalogID — {2}       |
...
| NotFound      | 404       | 800111     | Nie znaleziono subskrypcji platformy Azure o danym identyfikatorze uprawnień.       |
| Forbidden     | 403       | 800115     | Zamówienie jest już przypisane do innej dzierżawy. Skontaktuj się z klientem, aby rozwiązać pytania dotyczące własności.       |
| Forbidden     | 403       | 800114     | Nie masz uprawnienia do zarządzania nadgorą dla tego klienta.       |
| Forbidden     | 403       | 800112     | Nie można ustawić opłat za przesyt, ponieważ klient ma starsze subskrypcje platformy Azure.       |
...
| Forbidden     | 403       | 900130     | Niedawno zakupiono co najmniej jedną subskrypcję platformy Azure. W tej chwili nie można ich zmienić. Spróbuj ponownie później               |
...
| NotFound      | 404       | 900140     | Nie można uzyskać Azure Active Directory subskrypcji dla klienta z identyfikatorem dzierżawy i {0} identyfikatorem inicjowania obsługi źródła{1}     |
...
| BadRequest          | 400       | 600049     | Brakuje informacji o identyfikatorze rejestracji organizacji. Ten błąd jest zgłaszany, jeśli firma/organizacja klienta znajduje się w następujących krajach: Najcięższa(AM), Az( AZ), Ćwik (BY), Niemca), KZ, Kyrgyzstan(KG), Aimbion (MD), Anasja (RU), Tajikistan(TJ), Niechcący(UZ), UA), ale organizationRegistrationNumber nie zostanie przekazana.      |
...
| BadRequest          | 400       | 600051     | Brak numeru telefonu klienta. Ten błąd jest zgłaszany, jeśli firma/organizacja klienta znajduje się w następujących krajach: Najcięższa(AM), Az(AZ), Ćwikowa (HU), KZ), Kyrgyzstan(KG), Aichi (MD), Aai (RU), Tajikistan(TJ), Kod (UZ), UA), ale nie przekazano adresu billingProfile.defaultAddress.phoneNumber.       |
| BadRequest          | 400       | 600002     | Wartość identyfikatora rejestracji organizacji nie jest obsługiwana. Ten błąd jest zgłaszany, jeśli firma/organizacja klienta nie znajduje się w następujących krajach: Najcięższa(AM), Dolno(AZ), Łazik (HU), Korea (KZ), Kyrgyzstan(KG), Dominij (MD), Anasja (RU), Tajikistan(TJ), Stan (UZ), Stan Stanów Zjednoczonych), ale próbowano określić wartość organizationRegistrationNumber.       |
